...
auto it5 = temp.begin();
while (it5 != temp.end()) {
    if (std::find_if(e.begin(), e.end(), [&](const auto &p) { return p.first == it5->first && p.second == it5->second; }) == e.end()) {
        e.push_back(*it5);
    }
    it5++;
}
...
while (it6 != temp2.end()) {
    if (std::find_if(t.begin(), t.end(), [&](const auto &p) { return p.first == it6->first && p.second == it6->second; }) == t.end()) {
        t.push_back(*it6);
    }
    it6++;
}
